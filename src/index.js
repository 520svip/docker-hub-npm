...
export default {
	async fetch(request, env, ctx) {
		...
		if (env.UA) ua_black_list = ua_black_list.concat(await ADD(env.UA));
		...
		if (ua_black_list.some(fxxk => userAgent.includes(fxxk)) && ua_black_list.length > 0) {
			// 首页改成一个nginx伪装页
		...
		const conditions = [
			...
			pathname == '/c544f2d7540fc91cdd299e6a0f2b87ce.txt',
		];
		...
		if (conditions.some(condition => condition) && (fakePage === true || hostTop == 'docker')) {
			if (env.URL302) {
				...
				// 	},
				// });
			} else if (url.pathname == '/c544f2d7540fc91cdd299e6a0f2b87ce.txt') {
				return new Response("6e795ec427b474044f225f79d6519a03f0fd4cc0", {
					headers: {
						'Content-Type': 'text/html; charset=UTF-8',
					},
				});
